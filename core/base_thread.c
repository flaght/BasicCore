...
int base_threadattr_set_detach(base_threadattr_t *thrdattr, int on) {
  ...
  ret = pthread_attr_setdetachstate(
      &thrdattr->attr, on ? PTHREAD_CREATE_DETACHED : PTHREAD_CREATE_JOINABLE);
...
int base_init_thread(base_thread_t **thrd, base_threadattr_t *thrdattr,
                     base_thread_run_t thread_run_func, void *args) {
  int ret = 0;
  base_thread_t *_thrd;
