...
handler_t plugins_call_time_msg(struct server* srv, char* id, int opcode,
                                int time) {
  struct plugin** slot;
  size_t j;
  struct plugin* p;
...
static int plugins_register(struct server* srv, struct plugin* p) {
  ...
  if (srv->plugins.size == 0) {
    ...
    srv->plugins.size += 4;
    srv->plugins.ptr = (char*) realloc(srv->plugins.ptr,
                                       srv->plugins.size * sizeof(*p));
  }
  ...
  list_for_each(tmp,&srv->plugins_des)
  {
    ...
    if (pl_desc && (!(strcmp(pl_desc->id->ptr, "core") == 0))) {
      ...
      if (p->lib == NULL) {
        ...
        MIG_ERROR(USER_LEVEL, "dlopen()failed for:[%s] %s", pl_desc->id->ptr,
                  dlerror());
        plugin_free(p);
        return -1;
      }
      ...
      init = (int (*)(struct plugin*)) get_function_pointer(p->lib,
                                                            srv->tmp_buf->ptr);

      if (init == NULL) {
        MIG_ERROR(USER_LEVEL, "unable find %s in %s", srv->tmp_buf->ptr,
                  pl_desc->id->ptr);
        plugin_free(p);
        return -1;
      }
    ...
    if (p->init) {
      ...
      list_for_each(tmp,&srv->plugins_des)
      {
        ...
        if (pl_desc) {
          ...
          if ((strcmp(pl_desc->id->ptr, ((struct plugin_data*) (p->data))->id)
              == 0)
              && (strcmp(pl_desc->version->ptr,
                         ((struct plugin_data*) p->data)->version) == 0)) {
...
#undef COPY_PLUGIN
            MIG_ERROR(USER_LEVEL, "the plugin id[%s] name[%s] "
                      "version[%s] provider[%s]",
                      p->id->ptr, p->name->ptr, p->version->ptr,
                      p->provider->ptr);
            break;
          }
        }
