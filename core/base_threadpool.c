...
typedef struct base_thread_pool_task {
  BASE_RING_ENTRY(base_thread_pool_task)
  link;
...
struct base_thread_list_elt {
  BASE_RING_ENTRY(base_thread_list_elt)
  link;
  base_thread_t *thd;
...
static int thread_pool_construct(base_threadpool_t * me, size_t init_threads,
                                 size_t max_threads, size_t max_tasks) {
  int rv;
  int i;
...
static base_thread_pool_task_t *pop_task(base_threadpool_t * me) {
  ...
  if (task == me->task_idx[seg]) {
    ...
    if (me->task_idx[seg]
        == BASE_RING_SENTINEL(me->tasks, base_thread_pool_task, link)
...
static void * thread_pool_func(base_thread_t * t, void *param) {
  ...
  while (!me->terminated && elt->state != TH_STOP) {
    ...
    while (NULL != task && !me->terminated) {
      ...
      base_lock_threadmutex(me->lock);
      BASE_RING_INSERT_TAIL(me->recycled_tasks, task, base_thread_pool_task,
                            link);
      elt->current_owner = NULL;
    ...
    if ((me->idle_cnt >= me->idle_max
        && !(me->scheduled_task_cnt && 0 >= me->idle_max) && !me->idle_wait)
        || me->terminated || elt->state != TH_RUN) {
      --me->thd_cnt;
      if ((TH_PROBATION == elt->state) && me->idle_wait)
        ++me->thd_timed_out;
      BASE_RING_INSERT_TAIL(me->recycled_thds, elt, base_thread_list_elt, link);
      base_unlock_threadmutex(me->lock);
...
static base_thread_pool_task_t *task_new(base_threadpool_t * me,
                                         base_thread_run_t func, int flag,
                                         void *param, int priority, void *owner,
                                         microsecond_t time) {
...
static int schedule_task(base_threadpool_t *me, base_thread_run_t func,
                         int flag, void *param, void *owner, microsecond_t time) {
  base_thread_pool_task_t *t;
    ...
    if (t->dispatch.time < t_loc->dispatch.time) {
      ...
      if (t_loc
          == BASE_RING_SENTINEL(me->scheduled_tasks, base_thread_pool_task,
                                link)) {
        ++me->scheduled_task_cnt;
        BASE_RING_INSERT_TAIL(me->scheduled_tasks, t, base_thread_pool_task,
                              link);
        break;
      }
    }
...
static int add_task(base_threadpool_t *me, base_thread_run_t func, int flag,
                    void *param, int priority, int push, void *owner) {
  base_thread_pool_task_t *t;
  base_thread_pool_task_t *t_loc;
  base_thread_t *thd;
  int rv = 0;
  ...
  if (push) {
    while (BASE_RING_SENTINEL(me->tasks, base_thread_pool_task, link) != t_loc
        && t_loc->dispatch.priority >= t->dispatch.priority) {
      t_loc = BASE_RING_NEXT(t_loc, link);
    }
  }
  BASE_RING_INSERT_BEFORE(t_loc, t, link);
  ...
  FINAL_EXIT: me->task_cnt++;
  if (me->task_cnt > me->tasks_high)
    me->tasks_high = me->task_cnt;
  if (0 == me->thd_cnt
      || (0 == me->idle_cnt && me->thd_cnt < me->thd_max
...
static int remove_scheduled_tasks(base_threadpool_t *me, void *owner) {
  base_thread_pool_task_t *t_loc;
  base_thread_pool_task_t *next;

  t_loc = BASE_RING_FIRST(me->scheduled_tasks);
  while (t_loc
      != BASE_RING_SENTINEL(me->scheduled_tasks, base_thread_pool_task, link)) {
...
static size_t trim_idle_threads(base_threadpool_t *me, size_t cnt) {
  ...
  if (NULL != (head = elt)) {
    ...
    base_lock_threadmutex(me->lock);
    BASE_RING_SPLICE_TAIL(me->recycled_thds, head, tail, base_thread_list_elt,
                          link);
    base_unlock_threadmutex(me->lock);
  }
  assert(cnt == n_dbg);
...
int base_init_threadpool(base_threadpool_t ** me, size_t init_threads,
                         size_t max_threads, size_t max_tasks) {
  base_thread_t *t;
  int rv = 0;
...
// 添加一个优先任务
int base_threadpool_addtask(base_threadpool_t *me, base_threadpool_run_t func,
                            int flag, void *param, int priority, void *owner) {
  return add_task(me, func, flag, param, priority, 1, owner);
}

// 添加一个定时任务
int base_threadpool_scheduletask(base_threadpool_t *me,
                                 base_threadpool_run_t func, int flag,
                                 void *param, microsecond_t time, void *owner) {
  return schedule_task(me, func, flag, param, owner, time);
}

// 提升任务为本优先级最先
int base_threadpool_top(base_threadpool_t *me, base_threadpool_run_t func,
                        int flag, void *param, int priority, void *owner) {
  return add_task(me, func, flag, param, priority, 0, owner);
}

// 取消任务
int base_threadpool_cancel_tasks(base_threadpool_t *me, void *owner) {
  int rv = 0;
...
// 设置空闲线程数
size_t base_threadpool_set_idle_max(base_threadpool_t *me, size_t cnt) {
  me->idle_max = cnt;
...
// 设置最大线程数
size_t base_threadpool_set_thread_max(base_threadpool_t *me, size_t cnt) {
  unsigned int n;
...
size_t base_threadpool_set_threshold(base_threadpool_t *me, size_t val) {
  size_t ov;
