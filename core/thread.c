  ...
  switch (flag) {
    case PACKET: {
      ...
      if (packet->type == ACCEPT)
        plugins_call_handler_read(packet->srv, packet->sock, packet->buf->ptr,
                                  packet->buf->used - 1);
      else if (packet->type == CONNECT)
        plugins_call_handler_read_srv(packet->srv, packet->sock,
                                      packet->buf->ptr, packet->buf->used - 1);
...
static int user_addtask(struct server *srv, int fd, struct plugin *pl) {
  ...
  if (srv == NULL || pl == NULL)
    return 0;
  struct thread_adapter *thr_srv = (struct thread_adapter*) malloc(
      sizeof(*thr_srv));
  thr_srv->srv = srv;
  thr_srv->data = pl;
...
static int time_addtask(struct server* srv, void* data) {
  ...
  if (srv == NULL || data == NULL)
    return 0;
  struct time_task* task = (struct time_task*) data;
  struct time_task* new_task = (struct time_task*) malloc(
      sizeof(struct time_task));
...
int init_threadpool(struct server *srv) {
  ...
  ret = base_init_threadpool(&srv->thrp, 0, atoi(srv->srv_conf.max_worker->ptr),
                             0);
