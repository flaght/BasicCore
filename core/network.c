...
static inline struct list_head *sock_adapter_sock2hash(struct server *srv,
                                                       int sock) {
...
void free_sock_adapter(struct sock_adapter *sa, struct server *srv) {
  //adapter_count--;
    ...
    for (i = 0; i < packet->block_count; i++) {
      ...
      if (i == 0) {
        ...
        buffer_append_string_len(
            buf, block->address + packet->u.blocks.head_position, len);
        //printf("%*.*s\n", len-4, len-4,
        ...
        buffer_append_string_len(buf, block->address,
                                 packet->u.blocks.tail_offset);
      }
  ...
  if (packet->block_count == 1) {
    ...
    for (i = 0; i < packet->block_count; i++) {
      ...
      if (i == 0) {
        int len = block->length - packet->u.blocks.head_position;
        buffer_append_string_len(
            pack_buff->buf, block->address + packet->u.blocks.head_position,
            len);
        ...
        buffer_append_string_len(pack_buff->buf, block->address,
                                 packet->u.blocks.tail_offset);
...
int echo_forward(void *privates, int length, struct server *srv) {
...
#else
      spconn->psc_rx_nob_wanted = *(int*) (block->address
          + conn->prev_packet_offset);
#endif
  ...
  while (1) {
    ...
    if (*p != '\f' && conn->offset < block->length) {
      /* do nothing */
      break;
    } else {
      struct echo_packet *packet = conn->packet;
...
#else
    rc = read(conn->psc_sock, conn->psc_rx_buffer, conn->psc_rx_size);
#endif
    if (rc > 0) {
...
int sockbase_schedule(struct psock_sched *sched, struct server *srv) {
  ...
  while (1) {
    ...
    if (!list_empty(&sched->pss_rx_conns)) {
      conn = list_entry(sched->pss_rx_conns.next, struct psock_conn,
                        psc_rx_list);
    ...
    if (!list_empty(&sched->pss_tx_conns)) {
      conn = list_entry(sched->pss_tx_conns.next, struct psock_conn,
                        psc_tx_list);
      ...
      if (rc == -ENOMEM) {
        assert(0);
      } else if (conn->psc_tx_ready && !list_empty(&conn->psc_tx_queue)) {
...
struct sock_adapter* create_connect_socket(struct server* srv, const char* host,
                                           const char* port) {
...
#if defined (PIPE_WORK)
struct sock_adapter* create_out_pipe(struct server* srv, int sock) {
  int opt, rc;
  ...
  if (rc != 0) {
    MIG_ERROR(USER_LEVEL,"fcntl set sock non-block failed: %s \n", strerror(errno));
    return NULL;
  }
  ...
  conn->sock = sock;
  conn->flags = EV_READ | EV_PERSIST;
  event_set(&conn->ev, sock, EV_READ | EV_PERSIST, server_read_buffer, conn);
  rc = event_add(&conn->ev, NULL);
  if (rc != 0) {
    MIG_ERROR(USER_LEVEL, "event_add failed (%s)\n", strerror(errno));
    destroy_sock_conn(conn->conn, srv);
    free_sock_adapter(conn, srv);
    close(sock);
    return NULL;
...
struct sock_adapter* create_in_pipe(struct server* srv, int sock) {
  int opt, rc;
  struct sock_adapter * conn;
  struct psock_conn *sc;
  opt = fcntl(sock, F_GETFL, 0);
  ...
  conn->sock = sock;
  conn->flags = EV_READ | EV_PERSIST;
  /*event_set(&conn->ev, sock, EV_READ | EV_PERSIST, server_write_buffer, conn);
   rc = event_add(&conn->ev, NULL);
   if (rc != 0) {
   MIG_ERROR(USER_LEVEL, "event_add failed (%s)\n", strerror(errno));
   destroy_sock_conn(conn->conn, srv);
...
struct sock_adapter* create_pipes_socket(struct server* srv) {
  int fds[2];
  int rc, opt;
  struct sock_adapter* conn = NULL;
  if (pipe(fds)) {
    MIG_ERROR(USER_LEVEL,"pipe() failed, errno:%s\n",strerror(errno));
    return NULL;
  }

  //管道监听
  create_out_pipe(srv, fds[0]);

  //写入管道
  srv->sa = create_in_pipe(srv, fds[1]);
}
...
int network_register_fdevents(struct server *srv) {
...
#if defined (PIPE_WORK)
  srv->sa = create_pipes_socket(srv);
#endif
  if (srv->sa == NULL) {
    return -1;
  }
  ...
  event_set(&srv->sa->ev, srv->sa->sock, EV_READ | EV_PERSIST, server_accept,
            srv);
  ...
  list_for_each(tmp,&srv->srv_conf.remotes)
  {
    ...
    if ((tmp_sa->port != atol(p->port->ptr))) {
      ...
      MIG_ERROR(USER_LEVEL,
                "connect error sa port[%d] srv_conf.remotes port[%d]",
...
int create_connects(struct server *srv) {
  ...
  if (list_empty(&srv->srv_conf.remotes))
    return 0;
  srv->connect_pool = (struct sock_adapter**) calloc(
      srv->ncount_connect, sizeof(struct sock_adapter*));
  if (srv->connect_pool == NULL)
    return -1;
  list_for_each(tmp,&srv->srv_conf.remotes)
  {
