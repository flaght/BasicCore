...
int netaio_send(struct server *srv, struct netaio **aio,
                struct socket_adapter *sa, void *data, size_t len) {
  int rc;
...
int netaio_multicast(struct server *srv, struct list_head *listaio, int *done,
                     int *complet_nums, struct list_head *listsa, void *data,
                     size_t len) {
...
static ssize_t sendfull(int socket, const char *buf, size_t nbytes) {
  ssize_t amt = 0;
  ssize_t total = 0;
...
int net_multicast(struct server *srv, struct list_head *listsa, void *data,
                  size_t len) {
  int l;
  struct list_head *node;
  struct socket_adapter *sa;
