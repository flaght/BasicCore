...
SET_SRV_CONFIG(srv_config_path,srv_conf.process_path);
	...
	return;
        ...
        for(i=0;atts[i]!=0;i+=2){
            ...
            SET_SRV_CONFIG(log_facility, srv_conf.facility);SET_SRV_CONFIG(
    log_usr_file, srv_conf.usr_file);SET_SRV_CONFIG(log_sys_file,
                                                    srv_conf.sys_file);SET_SRV_CONFIG(
    log_err_file, srv_conf.error_file);}
	return;
    }
    
    else if(strcmp(name,app_config)==0){
        for(i=0;atts[i]!=0;i+=2){
            SET_SRV_CONFIG(app_config_max_worker,
                                                      srv_conf.max_worker);SET_SRV_CONFIG(
    app_config_max_fds, srv_conf.max_fds);SET_SRV_CONFIG(app_config_max_conns,
                                                         srv_conf.max_conns);}
	return;
    }
        ...
        for(i=0;atts[i]!=0;i+=2){
            SET_SRV_CONFIG(
    other_config_modules_dir, srv_conf.modules_dir);
}
return;
}
...
static int check_more_plugin(struct plugin_desc *pl_desc,
